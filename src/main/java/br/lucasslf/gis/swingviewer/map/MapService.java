 ...
 * Classe MapService
 */
public class MapService implements DynamicMapService {
    ...
    private Log log = LogFactory.getLog(MapService.class);
    private Extent exportExtent;
    private Double exportScale;
    private int exportDPI;
    private boolean visible = true;
    private MapConfiguration dynamicMapConfig;

    public MapService(URL url, String id, MapServerProxy service, String coordinateSystemCode) {
        this.url = url;
        this.id = id;
    ...
    public MapService(String url, String id, MapServerProxy service, String coordinateSystemCode) {
        try {
    ...
    public MapService(String url, float alpha, String id, MapServerProxy service, String coordinateSystemCode) {
        try {
            this.url = new URL(url);
            this.id = id;
            this.service = service;
    ...
    public MapService(URL url, float alpha, String id, MapServerProxy service, String coordinateSystemCode) {
        this.url = url;
        this.alpha = alpha;
        this.id = id;
        this.service = service;
    ...
    public List<Layer> getLayers() {
        return dynamicMapConfig.getLayers();
    }
    ...
    private void processConfigLoadEvent(MapLoadEvent e) {
        for (MapLoadListener mapLoadListener : mapLoadListeners) {
            mapLoadListener.configLoaded(e);
        }
    }
    ...
    private List<Integer> getVisibleLayers() {
        List<Integer> visibleLayers = new ArrayList<Integer>();
        for (Layer l : getLayers()) {
            if (l.isVisible() && l.isLeaf()) {
                visibleLayers.add(l.getId());
            }
        }
    ...
    public void asyncReload(long callTimeStamp, Extent visibleExtent, double scale, int dpi, Dimension screenSize) {
        ...
        Image image = service.export(url, visibleExtent.getXmin().doubleValue(), visibleExtent.getYmin().doubleValue(), rightSuperiorCorner.getX(), rightSuperiorCorner.getY(), screenSize, coordinateSystemCode, getVisibleLayers());
    ...
    private void loadLayersLegends() {
        List<LayerLegend> ll = service.getLayerLegends(this);
        for (LayerLegend l : ll) {
            getLayer(l.getLayerId()).setLegends(l.getLegend());
        }
    }
    ...
    private void loadLayers() {
        for (Layer l : getLayers()) {
            l.setLayerConfiguration(service.getLayerConfiguration(this, l.getId()));
            if(l.getParentLayerId() > -1){
                l.setParentLayer(getLayer(l.getParentLayerId()));
            }
        }
    }

    private void loadConfig() {
        dynamicMapConfig = service.getMapConfiguration(this);
        loadLayers();
        loadLayersLegends();
    ...
    public String toString() {
        return "DynamicMapService{" + "url=" + url + '}';
    }
}
